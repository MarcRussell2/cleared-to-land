...
// A mission's hint, except while the stall warning sounds above the flare: main.js gives a mission hint priority over
// its own, and its own at that moment is the one that matters ("lower the nose a little and add power").
const H = (fn) => (ctx) => (ctx.ac.aero.warning && !ctx.ac.onGround && ctx.ac.radioAlt > ctx.ac.flareZone() ? null : fn(ctx));
...
export const FAILURES_MISSIONS = [
  ...
  },
  {
    ...
    tips: [
      ...
      'Ground speed (GS, in the box with the G) plus the headwind is your airspeed. The stall warning and the AoA still tell the truth.',
  ...
  },
  {
    ...
    scoring: { type: 'runway' },
    hint: H((ctx) => (ctx.ac.onGround ? null : 'Roll with rudder (Q/E): yaw first, the bank follows. A little left pedal holds the jam.')),
  },
  {
    ...
    tips: [
      'A torch lights the standby airspeed and altimeter: 62 kt, and Bayfield is at sea level, so the altimeter is your height.',

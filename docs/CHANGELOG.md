...
| 7-8 | Condor and Hornet cockpits | Not run: the quota ran out again on the first attempt at pass 7 (retry 06:20). The placeholder interiors ship. Briefs ready. |
| 9 | Exhaust and effects look | Not run (quota). The engine-side exhaust (haze, throttle puff, jet soot, failure stream) ships with placeholder numbers. |

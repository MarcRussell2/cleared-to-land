// A copy of the website harness's looksheet.mjs (tools/ctl-shots) with one addition: the
// contents of CTL_LOOK_INJECT (JavaScript) run before the game boots, so a sheet can be shot
// with an engine switch flipped (e.g. window.CTL_NO_CELLGATE = 1) and diffed against the
// normal one. Everything else is the original; keep it in step with the website copy.
...
// sharp is the website repo's dependency, not this one's: point CTL_SHARP at its package folder.
const sharp = (await import(process.env.CTL_SHARP ? pathToFileURL(join(process.env.CTL_SHARP, 'lib', 'index.js')).href : 'sharp')).default;
...
function writePage(name, P) {
  ...
  const inject = [
    ...
    (process.env.CTL_LOOK_INJECT ? '  ' + process.env.CTL_LOOK_INJECT : ''),
